import { FaPause, FaTimes, FaCheck, FaPlus } from 'react-icons/fa'
import { connect } from 'react-redux'
import { Link } from 'wouter'
import { removeGroup } from '../store/actions/actionCreators'

import '../styles/Card.scss'

const Card = ({ name, done, later, undone, id, deleteGroup }) => (
	<div className='card grid grid-rows-3 py-2 pt-0'>
		<div className='pl-4 flex items-center mb-2'>
			<div
				className='w-4 h-4 bg-red-400 rounded-full cursor-pointer hover:bg-red-500 transition'
		...
		<div className='grid grid-cols-3 p-3 pt-0 gap-x-3'>
			...
			<div className='flex rounded-full justify-around items-center p-1 bg-gray-500 shadow-sm hover:shadow-lg'>
				<FaPause className='text-xl text-yellow-400' />
...
const mapDispatchToProps = dispatch => ({
	deleteGroup: name => dispatch(removeGroup(name)),
})
